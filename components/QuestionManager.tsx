      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-start">
          ...
          <div className={`lg:col-span-5 ${activeTab === 'create' ? 'block' : 'hidden lg:block'}`}>
             <motion.div 
               ...
               className={`rounded-2xl sm:rounded-3xl border shadow-sm overflow-hidden transition-colors ${
                'bg-white dark:bg-card border-zinc-200 dark:border-border/10'
             }`}>
                 <div className="p-6 sm:p-8 border-b border-zinc-100 dark:border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-zinc-50/50 dark:bg-white/[0.02]">
                   ...
                   <div className="flex items-center gap-2 w-full sm:w-auto">
                     <button 
                       ...
                       className={`flex-1 sm:flex-none p-2 sm:p-2.5 rounded-xl border transition-all flex items-center justify-center gap-2 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest ${
                         showAIPanel 
                ...
                <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6 sm:space-y-8">
                   ...
                   <button 
                      type="submit"
                      disabled={submitting}
                      className={`w-full py-4 sm:py-5 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-all mt-4 flex items-center justify-center gap-3 shadow-lg disabled:opacity-50 ${
                         'bg-zinc-900 dark:bg-[#BFFF00] text-white dark:text-black hover:opacity-90'
                      }`}
                   >
                      {submitting ? (
                   ...
                   <AnimatePresence mode="popLayout">
                      {questions.map((q: Question, i: number) => (
                         <motion.div 
                            ...
                            className={`group p-6 sm:p-8 rounded-2xl sm:rounded-3xl border transition-all ${
                              editingId === q.id 
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-8">
                               <div className="flex-1 space-y-3 sm:space-y-4 w-full">
                                  <div className="flex items-center gap-2 sm:gap-3">
                                     <span className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase tracking-widest ${
