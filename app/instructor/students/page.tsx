...
interface Student {
  ...
  lastActive: string;
  history: number[];
  logs: { action: string; difficulty: string; time: string; isCorrect: boolean; timeTaken: number }[];
...
export default function StudentManagementPage() {
  ...
  const [unreadStudents, setUnreadStudents] = useState<Set<string>>(new Set());
  const [isBlocking, setIsBlocking] = useState(false);
  const [instructorId, setInstructorId] = useState<string | null>(null);
  const { socket } = useSocket(instructorId || '');

  useEffect(() => {
    if (socket) {
      socket.on('receive-message', (msg: any) => {
        if (!isChatOpen || selectedStudent?.id !== msg.senderId) {
          setUnreadStudents(prev => new Set(prev).add(msg.senderId));
          toast(`New message from ${msg.senderName || 'student'}`, { icon: '💬' });
        }
      });
    }
    return () => { socket?.off('receive-message'); };
  }, [socket, isChatOpen, selectedStudent]);

  useEffect(() => {
    if (isChatOpen && selectedStudent) {
      setUnreadStudents(prev => {
      ...
      toast((t) => (
        <div className="flex flex-col gap-4 p-2 min-w-[280px]">
          <div className="space-y-1">
            <p className="text-xs font-black text-red-500 uppercase tracking-widest">Security Alert</p>
            <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 leading-tight">
              Block access for <span className="font-black">{student.name}</span>?
            </p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => {
                toast.dismiss(t.id);
                executeBlock(student, newStatus);
              }}
              className="flex-1 px-4 py-2 bg-red-500 text-white text-[10px] font-black uppercase rounded-lg hover:bg-red-600 transition-all"
            >
              Confirm Block
            </button>
            <button
              onClick={() => toast.dismiss(t.id)}
              className="flex-1 px-4 py-2 bg-zinc-100 dark:bg-white/10 text-zinc-500 text-[10px] font-black uppercase rounded-lg hover:bg-zinc-200 transition-all"
            >
              Cancel
            </button>
          </div>
  ...
  const executeBlock = async (student: Student, newStatus: boolean) => {
    setIsBlocking(true);
    const toastId = toast.loading(newStatus ? 'Securing perimeter...' : 'Restoring access...');
    
    try {
      const res = await fetch('/api/instructor/students/block', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId: student.id, isBlocked: newStatus })
      });
      
      if (res.ok) {
        const data = await res.json();
        setStudents(prev => prev.map(s => s.id === student.id ? { ...s, isBlocked: data.isBlocked } : s));
        setSelectedStudent(prev => prev?.id === student.id ? { ...prev, isBlocked: data.isBlocked } : prev);
  ...
  return (
    <div className="space-y-8 sm:space-y-12 pb-24">
                    ...
                    <td className="px-6 sm:px-10 py-6 sm:py-8">
                      <div className="flex items-center gap-4 sm:gap-6">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full bg-[#BFFF00] flex items-center justify-center text-black font-black text-xs relative">
                            {student.name.charAt(0)}
                            {unreadStudents.has(student.id) && (
                              <span className="absolute -top-1 -right-1 h-3 w-3 bg-red-500 rounded-full border-2 border-white dark:border-[#080808] animate-pulse" />
                            )}
                          </div>
                          <div className="space-y-0.5">
                            <p className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                              {student.name}
                              {unreadStudents.has(student.id) && (
                                <span className="text-[8px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-500 font-black uppercase">New Message</span>
                              )}
                            </p>
                            <p className="text-[10px] text-zinc-500 font-medium">{student.email}</p>
                          </div>
                        </div>
                        <div className="hidden sm:block h-1 w-1 rounded-full bg-zinc-300" />
                        <p className="text-[9px] sm:text-[10px] text-zinc-400 font-bold uppercase tracking-tighter">ID: {student.id.slice(-6)}</p>
                      </div>
                    </td>
                    <td className="px-6 sm:px-10 py-6 sm:py-8">
                      <div className={`inline-flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] ${student.status === 'Mastery' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.1)]' :
                        ...
                        <div className={`h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full ${student.status === 'Mastery' ? 'bg-purple-400 animate-pulse' :
      ...
      {mounted && createPortal(
                  ...
                  <div className="flex items-center gap-6 sm:gap-10">
                    ...
                    <div className="space-y-1.5 sm:space-y-3">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                        ...
                        <div className={`px-2.5 sm:px-4 py-1 rounded-full text-[8px] sm:text-[10px] font-black uppercase tracking-widest border ${selectedStudent.status === 'Mastery' ? 'bg-purple-500/10 text-purple-500 border-purple-500/20' :
                ...
                <div className="px-6 sm:px-12 pt-6 sm:pt-10 flex gap-4 sm:gap-8 border-b border-zinc-100 dark:border-white/5 overflow-x-auto no-scrollbar">
                  {[
                    ...
                    <button
                      ...
                      className={`pb-4 sm:pb-6 text-[9px] sm:text-xs font-black uppercase tracking-widest sm:tracking-[0.3em] flex items-center gap-2 sm:gap-3 transition-all relative shrink-0 ${activeTab === tab.id ? 'text-[#BFFF00]' : 'text-zinc-400'
                        }`}
                    >
                      {tab.icon}
                      {tab.label}
                          ...
                          {selectedStudent.logs?.length > 0 ? (
                            selectedStudent.logs.map((log, i) => (
                              <div key={i} className="p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] bg-zinc-50 dark:bg-white/[0.02] border border-zinc-100 dark:border-white/5 flex items-center justify-between transition-all">
                                <div className="flex items-center gap-4 sm:gap-6">
                                  <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl flex items-center justify-center ${log.isCorrect ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                    <ActivityIcon size={16} />
                                  </div>
                                  <div className="space-y-1">
                                    <p className="text-sm sm:text-lg font-bold text-zinc-900 dark:text-white leading-tight">{log.action}</p>
                                    <div className="flex items-center gap-2">
                                      <span className="text-[9px] sm:text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{timeAgo(log.time)}</span>
                                      <span className="text-[8px] px-1.5 py-0.5 rounded bg-zinc-200 dark:bg-white/5 text-zinc-500 font-black uppercase">{log.difficulty}</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="text-right shrink-0 ml-4">
                                  <p className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white">{log.timeTaken}s</p>
                                  <p className="text-[8px] sm:text-[9px] font-black text-zinc-500 uppercase">Duration</p>
                                </div>
                              </div>
                            ))
                          ) : (
                            <div className="p-12 text-center space-y-4">
                              <AlertCircle size={40} className="mx-auto text-zinc-200 dark:text-white/5" />
                              <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">No activity recorded yet</p>
                            </div>
                          )}
                        </div>
                      </motion.div>
                ...
                <div className="p-6 sm:p-10 px-6 sm:px-12 bg-zinc-50/50 dark:bg-white/[0.02] border-t border-zinc-100 dark:border-white/5 flex flex-col sm:flex-row justify-between items-center gap-8 sm:gap-0">
                  <div className="flex gap-3 sm:gap-4 w-full sm:w-auto">
                    <button 
                      onClick={() => {
                        if (!instructorId) {
                          toast.error('Secure channel not initialized. Please wait...');
                          return;
                        }
                        setIsChatOpen(true);
                        toast.success('Opening secure channel...');
                      }}
                      className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-zinc-900 text-white dark:bg-white dark:text-black text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all relative"
                    >
                      <Mail size={14} className="sm:size-4" />
                      Message
                      {selectedStudent && unreadStudents.has(selectedStudent.id) && (
                        <span className="absolute -top-1 -right-1 h-3 w-3 sm:h-4 sm:w-4 bg-red-500 rounded-full border-2 border-white dark:border-[#080808] animate-pulse" />
                      )}
                    </button>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 sm:gap-10 w-full sm:w-auto">
                    ...
                    <button 
                      onClick={() => selectedStudent && handleBlockToggle(selectedStudent)}
                      disabled={isBlocking}
                      className={`text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${
                        selectedStudent?.isBlocked 
                        ? 'text-emerald-500 hover:text-emerald-600' 
                        : 'text-red-500 hover:text-red-600'
                      }`}
                    >
                      {isBlocking ? 'Processing...' : selectedStudent?.isBlocked ? 'Restore Access' : 'Block Access'}
                    </button>
                  </div>
                </div>
              </motion.div>
        ...
        {selectedStudent && instructorId && mounted && createPortal(
          <ChatDialog 
            isOpen={isChatOpen}
